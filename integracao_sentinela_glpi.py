...
SISTEMAS = {
    ...
    3: {'nome': 'Tutor', 'modulos': ['Alunos']},
    4: {'nome': 'Accessus', 'modulos': ['Usuário de Setor', 'Supervisor', 'Secretário','Operacional', 'Magistrado']},
    5: {'nome': 'Jurisconsult', 'modulos': ['TJMA', 'TELETRABALHO', 'VEP_CNPJ', 'UNIDADE_MONITORAMENTO_CARCERARIO', 'SEJAP', 'SECRETÁRIOS JUDICIAIS-2G', 'SECRETÁRIOS JUDICIAIS - DISTRIBUIÇÃO',
    ...
    6: {'nome': 'ThemisSG Web', 'modulos': ['Consulta','Arquivo','Assessoria Jurídica da Presidência','Atendimento','Auditoria','Configuração',
    ...
    7: {'nome': 'AR Digital', 'modulos': ['SERVIDOR_UNIDADE']},
    8: {'nome': 'CIG', 'modulos': ['Público', 'Suporte']},
    9: {'nome': 'ENGEDOC', 'modulos': ['Requisição - Cadastro', 'Fiscal', 'Requisição - Diretor', 'Requisição - Gestor']},
    10: {'nome': 'App Portal do Servidor', 'modulos': ['Servidores', 'Magistrados Aposentados', 'Delegatarios']},
    11: {'nome': 'IDS', 'modulos': ['Servidor']},
    12: {'nome': 'Meritus', 'modulos': ['Público']},
    13: {'nome': 'Planus', 'modulos': ['Servidores']},
    14: {'nome': 'Materiales', 'modulos': ['Usuário de Setor']}
}
...
def configurar_ambientes():
    if getattr(sys, 'frozen', False):
        playwright_dir = os.path.join(os.path.dirname(sys.executable), 'playwright')
        os.environ['PLAYWRIGHT_BROWSERS_PATH'] = os.path.join(playwright_dir, 'browsers')
        if not os.path.exists(os.environ['PLAYWRIGHT_BROWSERS_PATH']):
            messagebox.showerror("Erro", "Navegadores do Playwright não encontrados!")
            sys.exit(1)

def obter_dados():
    """Obtém os dados de entrada usando interface gráfica simples (sem perguntar a data)"""
    root = tk.Tk()
    root.withdraw()
    ...
    if None in [dados['usuario'], dados['senha'], dados['matricula'], dados['autenticacao_codigo']]:
